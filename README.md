...
**v1.6.0（最新）**
- 🔎 **元数据核对升级**：DOI 精确命中时，除题名外**年份/卷/期/页**也与权威库逐项比对，任一不符自动标 ⚠️——防"文献真实但卷期页录错"
- 🈴 **中文文献覆盖提示**：OpenAlex 中文题名覆盖有限，真实中文文献可能判 ❌，用知网/万方人工确认即可；修复 OpenAlex 版本年份误报（2017 论文命中 2025 重印版）

**v1.5.0**
- 🔗 **DOI 可点击跳转**：文末参考文献条目的 DOI 渲染为可点击超链接，点击直达论文页面（External 链接，Word 实测可用，重跑幂等）；GB 格式 DOI 统一为 `https://doi.org/…` 规范形式
- 📑 **著者-出版年制同年同作者 a/b 后缀**：同一作者同年多篇自动按题名字母序加 2023a/2023b，正文标注与文末条目一致（GB/T 7714 规范）
- 🔎 **note 质量检查**：`verify_refs.py` 新增 note 过短（<6 字）提示，确保写明支撑的具体句子/观点
- 📋 **最高规格自查清单**：SKILL.md 新增「5d 交稿前自查清单」——真实性 / 支撑性 / 无编造 / 无多余遗漏 / 格式 / 标注 / 排序 / 时效 / 可追溯 9 项逐条核对
...
> 完整版本线（v1.0.0 → v1.6.0）见 [CHANGELOG.md](CHANGELOG.md)。
